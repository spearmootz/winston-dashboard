...
import './index.css';
import { Container, Row, Col, Input, InputGroup, InputGroupAddon, InputGroupText, Pagination, PaginationItem, PaginationLink, Table } from 'reactstrap';
...
class App extends Component {
  ...
  get sourceSelector() {
    return (
      <InputGroup>
        ...
        <Dropdown onChange={source => this.updateState({ source, page: 1 })} value={this.state.source} options={this.state.sources} label='Select Log File' />
      </InputGroup>
    );
  }
  ...
  get rowsSelector() {
    return (
      <InputGroup>
        ...
        <Dropdown onChange={rows => this.updateState({ rows, page: 1 })} value={this.state.rows} options={rowOptions} label='Select Log File' />
      </InputGroup>
    );
  }
  ...
  get levelSelector() {
    return (
      <InputGroup>
        ...
        <Dropdown onChange={this.selectLevelFilter} value={this.getLevel(this.state.levelFilter).label} options={levelNames.map(level => levels[level].label)} label='Select Minimum Filter Level' />
      </InputGroup>
    );
  }
  ...
  get logs() {
    ...
    const logs = levelFilter === 'silly' ?
      this.state.logs :
      this.state.logs.filter(({ level }) => this.getLevel(level).value <= levelFilterValue);

    return inputFilter == '' ? logs : logs.filter(log => log.message.toLowerCase().includes(textFilter));
  }
      ...
      <Row className='justify-content-center'>
        <Col md='6'>
          <Pagination className='justify-content-center'>
            ...
            <PaginationItem><PaginationLink >Page {this.state.page}</PaginationLink></PaginationItem>
            {
  ...
  get inputFilter() {
    return (
      <InputGroup>
        ...
        <Input placeholder="Search" value={this.state.inputFilter} onChange={this.changeInputFilter} />
      </InputGroup>
    );
  }
